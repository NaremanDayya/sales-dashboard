...
text/x-generic table.blade.php ( HTML document, UTF-8 Unicode text )
            ...
            <div class="table-filters">
                ...
                <div class="filter-section">
                    ...
                    <div class="filter-row">
                        ...
                        <div class="search-box">
                            <input type="text" class="search-input" placeholder="ابحث في اسم الشركة، العنوان، الهاتف..." id="searchInput">
                            <i class="fas fa-search search-icon"></i>
                        </div>

                        <!-- Status Filter -->
                        <div class="filter-group">
                            <span class="filter-label">حالة العميل:</span>
                            <select id="filterSelect" onchange="applyFilter()" class="form-select" style="font-weight: 700; font-size: 14px; min-width: 120px;">
                                ...
                                <option value="not interested" {{ request('interest_status') == 'not_interested' ? 'selected' : '' }}>غير مهتم</option>
                                <option value="late" {{ request('filter') == 'late' ? 'selected' : '' }}>متأخرين</option>
                                <option value="late_interested" {{ request('filter') == 'late_interested' ? 'selected' : '' }}>متأخرين مهتمين</option>
                                <option value="late_not_interested" {{ request('filter') == 'late_not_interested' ? 'selected' : '' }}>متأخرين غير مهتمين</option>
                                <option value="late_neutral" {{ request('filter') == 'late_neutral' ? 'selected' : '' }}>متأخرين مؤجلين</option>
                            </select>
                        </div>

                        <!-- Service Filter -->
                        <div class="filter-group">
                            <span class="filter-label">الخدمة:</span>
                        ...
                        <!-- Sales Rep Filter -->
                        <div class="filter-group">
                            <span class="filter-label">مندوب المبيعات:</span>
                ...
                <div class="filter-section">
                    ...
                    <div class="filter-row">
                        ...
                        <div class="filter-group">
                            <span class="filter-label">تاريخ الإنشاء:</span>
                            <div class="date-filter-group">
                                <input type="text" id="createdAtFilter" class="form-input date-input" placeholder="اختر التاريخ">
                                <span id="createdAtCount" class="count-badge" style="display: none;">0</span>
                                <button onclick="filterByCreatedDate()" class="filter-btn apply">
                                    <i class="fas fa-filter"></i> تطبيق
                                </button>
                                <button onclick="resetCreatedDateFilter()" class="filter-btn reset">
                        ...
                        <!-- Contact Date Range -->
                        <div class="filter-group">
                            <span class="filter-label">تاريخ آخر تواصل:</span>
                            <div class="date-filter-group">
                                ...
                                <button onclick="filterByDate()" class="filter-btn apply">
                                    <i class="fas fa-filter"></i> تطبيق
                                </button>
                                <button onclick="resetDateFilter()" class="filter-btn reset">
                                    <i class="fas fa-times"></i> إعادة
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="filter-row">
                            <div class="filter-group">
                                <div x-data="{ open: false }">
                                    <button @click="open = true" class="filter-btn" style="background: #f59e0b; color: white;">
                                        <i class="fas fa-cog"></i> أيام التأخير
                                    </button>
                            ...
                            <!-- Reset All Filters -->
                            <div class="filter-group">
                                <button onclick="resetAllFilters()" class="filter-btn reset" style="background: #ef4444; color: white;">
        ...
        @push('scripts')
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (serviceId) document.getElementById('serviceTypeFilter').value = serviceId;
                    if (salesRep) document.getElementById('salesRepFilter').value = salesRep;

                    // Apply filters if any parameters exist
                    if (filter || serviceId || salesRep) {
                        applyFilter();
                    }
                ...
                function applyAllFilters() {
                    ...
                    if (serviceFilter) {
                        filteredData = applyServiceFilter(filteredData, serviceFilter);
                    }

                    // Apply sales rep filter
                    if (salesRepFilter) {
                        filteredData = filteredData.filter(client =>
                            client.sales_rep_name === salesRepFilter
                        );
                    }
                ...
                function applyStatusFilter(data, status) {
                    switch (status.toLowerCase()) {
                        ...
                        case 'late':
                            return data.filter(client => client.is_late_customer === true);
                ...
                function applyServiceFilter(data, serviceId) {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    document.getElementById('filterSelect').addEventListener('change', applyAllFilters);
                    document.getElementById('serviceTypeFilter').addEventListener('change', applyAllFilters);
                    document.getElementById('salesRepFilter').addEventListener('change', applyAllFilters);
                ...
                function filterByDate() {
                    ...
                    const filteredClients = currentFilteredClients.filter(client => {
                    ...
                    if (filteredClients.length === 0) {
                        showNotification('لا توجد نتائج في الفترة المحددة', 'error');
                        return;
                    }

                    currentFilteredClients = filteredClients;
                    renderTable(currentFilteredClients);
                    showNotification('تم تطبيق تصفية التاريخ', 'success');
                }
                ...
                function filterByCreatedDate() {
                    ...
                    if (!createdAtDate) {
                        showNotification('الرجاء تحديد تاريخ الإنشاء', 'error');
                        return;
                    }
                    ...
                    if (filteredClients.length === 0) {
                        showNotification('لا توجد عملاء تم إنشاؤهم في هذا التاريخ', 'error');
                        return;
                    }

                    currentFilteredClients = filteredClients;
                    renderTable(currentFilteredClients);
                    showNotification('تم تطبيق تصفية تاريخ الإنشاء', 'success');
                }
                ...
                function resetDateFilter() {
                    ...
                    document.getElementById('toDate').value = '';
                    applyAllFilters();
                    showNotification('تم إعادة تعيين تصفية التاريخ', 'success');
                }
                ...
                function resetCreatedDateFilter() {
                    ...
                    hideCreatedAtCount();
                    applyAllFilters();
                    showNotification('تم إعادة تعيين تصفية تاريخ الإنشاء', 'success');
                }
    ...
    ` : ''}
        ...
        `;
                ...
                function applyFilter() {
                    ...
                    if (criteria) urlParams.set('filter', criteria);
                    if (serviceId) urlParams.set('service_id', serviceId);
                    if (salesRepName) urlParams.set('sales_rep', salesRepName);
