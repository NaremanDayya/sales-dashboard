<div class="max-w-4xl mx-auto bg-white rounded-xl shadow-md overflow-hidden p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-2">
            ...
            <select id="client_id" name="client_id" required
                ...
                <option value="{{ $agreement->client->id }}" selected>
                    {{ $agreement->client->company_name }} (Not in list)
                </option>
                @endif
        ...
        <div class="space-y-2">
            ...
            <select id="service_id" name="service_id" required
                ...
                <option value="{{ $service->id }}" {{ old('service_id', $agreement->service_id ?? '') == $service->id ?
                    'selected' : '' }}>
                    {{ $service->name }}
                </option>
                @endforeach
    ...
    <div class="bg-gray-50 p-4 rounded-lg">
        <h3 class="text-lg font-medium text-gray-800 mb-4">Agreement Dates</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Signing Date -->
            <div class="space-y-2">
                <label for="signing_date" class="block text-sm font-medium text-gray-700">Signing Date</label>
            ...
            <!-- Implementation Date -->
            <div class="space-y-2">
                <label for="implementation_date" class="block text-sm font-medium text-gray-700">Implementation
                    Date</label>
        ...
        <div class="mt-4 space-y-2">
            ...
            <input type="text" id="expiration_date"
                ...
                value="{{ isset($agreement) && $agreement->signing_date ? $agreement->signing_date->addYears($agreement->duration_years)->format('Y-m-d') : '' }}">
        </div>
    </div>

    <!-- Terms Section -->
    <div class="bg-gray-50 p-4 rounded-lg">
        <h3 class="text-lg font-medium text-gray-800 mb-4">Agreement Terms</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Termination Type -->
            <div class="space-y-2">
                <label for="termination_type" class="block text-sm font-medium text-gray-700">Termination Type</label>
            ...
            <!-- Notice Period -->
            <div class="space-y-2">
                <label for="notice_months" class="block text-sm font-medium text-gray-700">Notice Period
                    (Months)</label>
            ...
            <!-- Agreement Status -->
            <div class="space-y-2">
                <label for="agreement_status" class="block text-sm font-medium text-gray-700">Agreement Status</label>
                <select id="agreement_status" name="agreement_status" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border">
                    <option value="active" {{ old('agreement_status', $agreement->agreement_status ?? '') == 'active' ?
                        'selected' : '' }}>
                        Active
                    </option>
                    ...
                    <option value="pending" {{ old('agreement_status', $agreement->agreement_status ?? '') == 'pending'
                        ? 'selected' : '' }}>
                        Pending
                    </option>
                </select>
        ...
        <div id="return_value_container" class="mt-4 space-y-2"
            style="display: {{ old('termination_type', $agreement->termination_type ?? '') == 'returnable' ? 'block' : 'none' }};">
            <label for="return_value" class="block text-sm font-medium text-gray-700">Return Value Amount</label>
            <div class="mt-1 relative rounded-md shadow-sm">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span class="text-gray-500 sm:text-sm">SAR</span>
                </div>
                <input type="number" id="return_value" name="return_value"
                    value="{{ old('return_value', $agreement->return_value ?? 0) }}" min="0" step="0.01"
            ...
            <p class="mt-1 text-sm text-gray-500">Amount to be returned upon termination</p>
            @error('return_value')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
    ...
    <!-- Financial Section -->
    <div class="bg-gray-50 p-4 rounded-lg">
        <h3 class="text-lg font-medium text-gray-800 mb-4">Financial Details</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Price per Service -->
            <div class="space-y-2">
                <label for="price" class="block text-sm font-medium text-gray-700">Price per Service</label>
                <div class="mt-1 relative rounded-md shadow-sm">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span class="text-gray-500 sm:text-sm">SAR</span>
                    </div>
                    <input type="number" id="price" name="price" value="{{ old('price', $agreement->price ?? '') }}"
                        required step="0.01"
                        class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-12 pr-12 sm:text-sm border-gray-300 rounded-md p-3 border">
                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        <span class="text-gray-500 sm:text-sm">.00</span>
        ...
        <div class="mt-4 space-y-2">
            <label class="block text-sm font-medium text-gray-700">Calculated Total Amount</label>
            <div class="mt-1 relative rounded-md shadow-sm">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span class="text-gray-500 sm:text-sm">SAR</span>
                </div>
                <input type="text" id="display_total_amount"
                    ...
                    readonly value="{{ isset($agreement) ? number_format($agreement->total_amount, 2) : '0.00' }}">
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const returnValueContainer = document.getElementById('return_value_container');

        terminationType.addEventListener('change', function() {
            returnValueContainer.style.display = this.value === 'returnable' ? 'block' : 'none';
        });
        ...
        function calculateExpirationDate() {
            if (signingDate.value && durationInput.value) {
                const date = new Date(signingDate.value);
                date.setFullYear(date.getFullYear() + parseInt(durationInput.value));
                expirationDateInput.value = date.toISOString().split('T')[0];
            } else {
