...
const AgreementsData = @json($Agreements);

function renderTable(data = AgreementsData) {
    ...
    data.forEach(agreement => {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-2 text-sm text-gray-700">${agreement.termination_type || '—'}</td>
            ...
            <td class="px-4 py-2 text-sm text-center">
                <span class="inline-block px-2 py-0.5 rounded-full ${
                    agreement.notice_status === 'sent'
                        ...
                        : agreement.notice_status === 'not_sent'
...
document.getElementById('searchInput').addEventListener('input', function(e) {
            ...
            const filteredData = AgreementsData.filter(agreement => {
            return (
                (agreement.contact_person && agreement.contact_person.toLowerCase().includes(searchTerm)) ||
                (agreement.company_name && agreement.company_name.toLowerCase().includes(searchTerm)) ||
                (agreement.interest_status && agreement.interest_status.toLowerCase().includes(searchTerm)) ||
                (agreement.response_status && agreement.response_status.toLowerCase().includes(searchTerm))||
                (agreement.phone && agreement.phone.toLowerCase().includes(searchTerm))
...
function applyFilter() {
    ...
    if (!criteria) {
        currentFilteredAgreements = [...AgreementsData];  // reset filter
        renderTable(currentFilteredAgreements);
        return;
    }
    ...
    break;
      case 'not sent':
    currentFilteredAgreements = AgreementsData.filter(agreement =>
        agreement.notice_status === 'not sent'
    );
    break;
        case 'returnable':
            currentFilteredAgreements = AgreementsData.filter(agreement => agreement.termination_type === 'returnable');
            break;

        case 'non returnable':
            currentFilteredAgreements = AgreementsData.filter(agreement => agreement.termination_type === 'non returnable');
            break;
