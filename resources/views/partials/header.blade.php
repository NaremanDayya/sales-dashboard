        ...
        <nav class="nav flex-row items-center gap-1 mx-4 hidden lg:flex">
            @if(Auth::user()->hasRole('admin'))
                ...
                <a href="{{ route('admin.team.index') }}"
                   class="nav-link px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors duration-200 {{ request()->routeIs('admin.team.*') ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-white' : 'text-gray-700 dark:text-gray-300' }}">
    ...
    chatDropdownOpen: false }"
            ...
            <div x-data="{ notifDropdownOpen: false }" class="relative">
                <button @click="notifDropdownOpen = !notifDropdownOpen"
                    ...
                    @if($unreadCount > 0)
                        <span class="absolute -top-1 -left-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center transform -translate-x-1/2 -translate-y-1/2 animate-pulse">
                            {{ $unreadCount }}
                        </span>
                    @endif
                </button>
