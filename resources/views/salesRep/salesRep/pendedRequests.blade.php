...
@php
    ...
    $classes = $statusClasses[$request->status] ?? 'bg-gray-100 text-gray-800';
@endphp

<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $classes }}" style="font-weight:800;">    @php
        $statusTranslations = [
            'approved' => 'مقبول',
            'rejected' => 'مرفوض', 
            'pending' => 'قيد الانتظار'
        ];
        echo $statusTranslations[$request->status] ?? $request->status;
    @endphp
</span></td>
...
@if(auth()->user()->role == 'admin')
                        <td class="no-print">
                            <div class="flex space-x-2">
                                ...
                                @endphp
                                <a href="{{ $route }}"
                                    class="px-3 py-1 bg-green-100 text-green-800 rounded-md text-sm hover:bg-green-200 transition-colors">                                    تقييم
                                </a>
                            </div>
                        </td>
@endif
...
طلب خاص بالعميل
    ...
    <td>
        @php
            ...
            $classes = $statusClasses[$request->status] ?? 'bg-gray-100 text-gray-800';
        @endphp
        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $classes }}" style="font-weight:800;">
            @php
                $statusTranslations = [
                    'approved' => 'مقبول',
                    'rejected' => 'مرفوض',
...
@if(auth()->user()->role === 'admin')
  <td class="px-3 py-2 text-sm text-center no-print">
        <div class="flex space-x-2">
 <a href="{{ route('admin.chat-client-request.review', [
                        'client' => $request->client_id,
                        'client_request' => $request->id
                    ]); }}" 
...
@endforeach
                    ...
                    <tr>
<td style="font-size: 14px; font-weight: 700;">
    <a href="{{ route('admin.agreement-request.review', ['agreement' => $request->agreement_id, 'agreement_request' => $request->id]) }}"       class="text-gray-800 hover:text-primary hover:underline">
        طلب تعديل اتفاقية
    </a>
</td>
                        <td>
...
@if($clientRequests->isEmpty() && $agreementRequests->isEmpty() && $chatClientRequests->isEmpty())
            <tr>
                <td colspan="@if(auth()->user()->role == 'admin') 8 @else 7 @endif" class="empty-state">
