...
@section('content')
<x-page-header title="مراجعة طلب العميل من المحادثة" subtitle="طلب وارد من محادثة العميل">
    <x-slot name="actions">
        @php
            $statusColor = match($client_request->status) { 'pending' => 'amber', 'approved' => 'emerald', 'rejected' => 'rose', default => 'gray' };
        @endphp
        <x-badge :color="$statusColor">{{ ucfirst($client_request->status) }}</x-badge>
    </x-slot>
</x-page-header>
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h5 class="text-sm font-semibold text-gray-900 flex items-center gap-2">
                <i class="bi bi-building text-indigo-500"></i>{{ $client_request->client->company_name }}
            </h5>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-gray-900">طلب من العميل (محادثة)</p>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div>
                        <h6 class="text-xs font-medium text-gray-400 mb-0.5">سفير العلامة التجارية</h6>
            ...
            <div class="bg-indigo-50/60 rounded-lg p-4 my-4">
                <h6 class="text-sm font-semibold text-indigo-700 mb-2 flex items-center gap-2"><i class="bi bi-chat-dots"></i>تفاصيل الطلب</h6>
                <p class="text-sm text-gray-700">{{ $client_request->message }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h5 class="text-sm font-semibold text-gray-900 flex items-center gap-2"><i class="bi bi-clipboard2-check text-indigo-500"></i>تقييم الطلب</h5>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('admin.chat-client-request.update', [$client_request->client_id, $client_request->id]) }}">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <label for="status-approved" class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition-colors has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50 border-gray-200">
                                ...
                                <span class="flex items-center gap-3">
                                    ...
                                    <span>
                                        ...
                                        <small class="text-xs text-gray-500">سيتم تنفيذ الطلب أو الرد عليه</small>
                                    </span>
                                </span>
                            </label>
                            ...
                            <label for="status-rejected" class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition-colors has-[:checked]:border-rose-500 has-[:checked]:bg-rose-50 border-gray-200">
                                ...
                                <span class="flex items-center gap-3">
                                    ...
                                    <span>
                                        ...
                                        <small class="text-xs text-gray-500">لن يتم اتخاذ إجراء</small>
                                    </span>
                                </span>
                            </label>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="6" placeholder="ملاحظات الإدارة"
                        ...
                        <p class="mt-1.5 text-xs text-gray-400">أضف أي ملاحظات للمندوب بخصوص هذا الطلب</p>
                    </div>
                </div>
